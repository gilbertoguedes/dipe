...
class User_auth {
    ...
    {
        $CI =& get_instance();
        $result = $this->check_valid_email($email,$code);
        if ($result)
        {
            $key = md5(time());
            $key = str_replace("1", "z", $key);
	...
	public function login_password_cripto($email,$password)
	{
		$CI =& get_instance();
		$result = $this->check_valid_user_password_cripto($email,$password);
		if (count($result) == 1)
		{
            $key = md5(time());
			$key = str_replace("1", "z", $key);
	...
	{ 	
        ...
        $CI->db->where(array('customer_email'=>$email,'password'=>$password,'status'=>1));
    ...
    //Check valid user
    function check_valid_email($email,$code)
    {
        $CI 		=& get_instance();
        $CI->db->where(array('customer_email'=>$email,'reset_password_code'=>$code,'status'=>1));
        $query 		= $CI->db->get('customer_information');
        $result 	=  $query->result_array();
	...
	//Check valid user
	function check_valid_user_password_cripto($email,$password)
	{
        $CI 		=& get_instance();
		$CI->db->where(array('customer_email'=>$email,'password'=>$password,'status'=>1));
		$query 		= $CI->db->get('customer_information');
	...
	{ 	
		$CI 		=& get_instance();
		$query = $CI->db->where(array('customer_id'=>$user_id,'status'=>2))
        ...
        if (count($result) == 1)
		{
            ...
            $CI->db->update('customer_information',$data);
	...
	//Check valid user
	function activate_user($user_id,$data)
	{ 	
		$CI 		=& get_instance();
		$CI->db->where('customer_id', $user_id);
		$CI->db->update('customer_information', $data); 
