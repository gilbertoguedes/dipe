...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:catalogocuentas="www.sat.gob.mx/esquemas/ContabilidadE/1_1/CatalogoCuentas" xmlns:catalogocuentasB="http://www.sat.gob.mx/esquemas/ContabilidadE/1_1/CatalogoCuentas">
<!--En esta sección se define la inclusión de las plantillas de utilerías para colapsar espacios -->
  <xsl:include href="http://www.sat.gob.mx/esquemas/utilerias.xslt"/>
	<!-- Con el siguiente método se establece que la salida deberá ser en texto -->
	<xsl:output method="text" version="1.0" encoding="UTF-8" indent="no"/>
  <!-- Aquí iniciamos el procesamiento de la cadena original con su | inicial y el terminador || -->
  <xsl:template match="/">|<xsl:apply-templates select="/catalogocuentas:Catalogo"/><xsl:apply-templates select="/catalogocuentasB:Catalogo"/>||</xsl:template>
 ...
 <!-- Sección B -->	
	<xsl:template match="catalogocuentasB:Catalogo">
		<xsl:call-template name="Requerido">
			<xsl:with-param name="valor" select="./@Version"/>
		</xsl:call-template>
		<xsl:call-template name="Requerido">
		...
		<xsl:apply-templates select="./catalogocuentasB:Ctas"/>
	</xsl:template>
	<xsl:template match="catalogocuentasB:Ctas">
		<xsl:call-template name="Requerido">
			<xsl:with-param name="valor" select="./@CodAgrup"/>
		</xsl:call-template>
		<xsl:call-template name="Requerido">
