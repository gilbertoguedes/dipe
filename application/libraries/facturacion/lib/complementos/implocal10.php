...
function mf_complemento_implocal10($datos)
{
    ...
    {
        foreach($datos['TrasladosLocales'] as $idx => $entidad)
        {
            $atrsentidad = mf_atributos_nodo($entidad);
            $xml .= "<implocal:TrasladosLocales $atrsentidad/>";
            
        }
    }
	if(isset($datos['RetencionesLocales']))
    {
        foreach($datos['RetencionesLocales'] as $idx => $entidad)
        {
            $atrsentidad = mf_atributos_nodo($entidad);
            $xml .= "<implocal:RetencionesLocales $atrsentidad/>";
